...
(async()=>{
 ...
 for(const variant of (baseline ? ['before','after'] : ['after'])) {
  ...
  const result=await page.evaluate(async()=>{
   ...
   return {retainedRoad:road===roadBefore,retainedGeometry:road.getLatLngs()===geometryBefore,start,end:count(),signatures,zoom,pan,zoomScript,panScript,created,patterns,rotation,popup:document.querySelector('.leaflet-popup-content')?.textContent};
  });
  ...
  if(variant==='after'){
   assert.equal(result.retainedRoad,true);
   assert.equal(result.retainedGeometry,true);
   assert(result.created.every(count=>count===0),'Zoom must not recreate layers');
   assert(result.patterns.every(count=>count===result.start.patterns),'Pattern definitions must remain bounded');
  }
  results.push({variant,sizeKm,displayMode,...result,errors});console.log(JSON.stringify({...results.at(-1),signatures:undefined,popup:Boolean(result.popup)}));
  await context.close();
 }
 fs.writeFileSync(path.join(assets,`results-${sizeKm}-${displayMode}.json`),JSON.stringify(results,null,2));
 if(baseline) assert.deepEqual(results[1].signatures,results[0].signatures,'Symbol sizes/styles must match the original at every zoom');
 await browser.close();
