...
(async () => {
  ...
  try {
    ...
    await owner.page.locator('#teamMemberForm input').fill('editor');await owner.page.locator('#teamMemberForm button').click();await owner.page.getByText('editor (editor) · Redigerare').waitFor();
    await owner.page.locator('#teamMemberForm input').fill('viewer');await owner.page.locator('#teamMemberForm select').selectOption('viewer');await owner.page.locator('#teamMemberForm button').click();await owner.page.getByText('viewer (viewer) · Läsare').waitFor();
    ...
    await viewer.page.setViewportSize({width:390,height:844});await viewer.page.locator('#teamSyncButton').click();await viewer.page.screenshot({path:'.test-artifacts/team-mobile.png'});
    ...
    await viewer.page.locator('[data-team-rescue]').click();await viewer.page.getByText('1 kartobjekt kopierades',{exact:false}).waitFor();
