...
function renderPoint(o){const m=L.marker([o.coordinates[1],o.coordinates[0]],{icon:pointIcon(o.objectType||o.type),draggable:mode==='manual'}).addTo(map);m.on('dragend',e=>{const p=e.target.getLatLng();o.coordinates=[p.lng,p.lat];o.source='manual-adjustment';save()});m.on('click',()=>selectFeature('point',o,m));layers.set(o.id,m)}
function renderLine(o){const l=L.polyline(latlngs(o.coordinates),lineStyle(o.objectType||o.type)).addTo(map);l.on('click',()=>selectFeature('line',o,l));layers.set(o.id,l)}
function renderArea(o){const p=L.polygon(latlngs(o.coordinates),areaStyle(o.objectType||o.type)).addTo(map);p.on('click',()=>selectFeature('area',o,p));layers.set(o.id,p)}
