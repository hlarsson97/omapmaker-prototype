(function(root){
  ...
  function vectorElements(feature,context){
    ...
    if(d.kind==='bridge-tunnel')for(const line of coordinatesForGeometry(feature.geometry)){const projected=line.map(p=>paperProject(p,context));if(projected.length<2)continue;for(const [point,other] of [[projected[0],projected[1]],[projected.at(-1),projected.at(-2)]]){const tangent=Math.atan2(point.y-other.y,point.x-other.x),normal=tangent+Math.PI/2,half=lineWidth(d.tagLengthMm)/2,along=lineWidth(d.tagSpacingMm)/2;for(const side of [-1,1]){const cx=point.x+Math.cos(tangent)*along,cy=point.y+Math.sin(tangent)*along,dx=Math.cos(normal)*half,dy=Math.sin(normal)*half;elements.push({layer:layer(d.colour),markup:`<path data-decoration-symbol="${symbol}" d="M${cx-dx},${cy-dy} Q${point.x},${point.y} ${cx+dx},${cy+dy}" fill="none" stroke="${stroke(d.colour)}" stroke-width="${lineWidth(d.tagWidthMm)}"/>`})}}}
