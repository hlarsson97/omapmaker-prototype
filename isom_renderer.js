(function(root){
  ...
  function vectorElements(feature,context){
    ...
    if(d.kind==='bridge-tunnel')for(const line of coordinatesForGeometry(feature.geometry)){const projected=line.map(p=>paperProject(p,context));if(projected.length<2)continue;const tag=lineWidth(d.tagLengthMm),angle=number(d.tagAngleDeg||60)*Math.PI/180,along=Math.cos(angle)*tag,across=Math.sin(angle)*tag;for(const [point,inside,isStart] of [[projected[0],projected[1],true],[projected.at(-1),projected.at(-2),false]]){const dx=isStart?inside.x-point.x:point.x-inside.x,dy=isStart?inside.y-point.y:point.y-inside.y,length=Math.hypot(dx,dy)||1,axis={x:dx/length,y:dy/length},normal={x:-axis.y,y:axis.x},outward=isStart?-1:1,outer={x:point.x+axis.x*outward*along+normal.x*across,y:point.y+axis.y*outward*along+normal.y*across};elements.push({layer:layer(d.colour),markup:`<path data-decoration-symbol="${symbol}" d="M${point.x},${point.y} L${outer.x},${outer.y}" fill="none" stroke="${stroke(d.colour)}" stroke-width="${lineWidth(d.tagWidthMm||d.widthMm)}" stroke-linecap="butt"/>`})}}
