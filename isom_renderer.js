(function(root){
  ...
  function preflight(features,options){
    ...
    (features||[]).forEach((feature,index)=>{
      ...
      if(d.requiresDirection&&!feature.properties?.[directionProperty])issues.push({severity:'warning',code:'direction-missing',symbol,featureId:feature.id||null,message:directionProperty==='tagSide'?`${label}: taggsida saknas, så staketets taggar kan inte placeras säkert.`:`${label}: fallriktning saknas, så taggar kan inte riktas säkert.`});
  ...
  function vectorElements(feature,context){
    ...
    if(d.kind==='styled-line')for(const line of coordinatesForGeometry(feature.geometry))for(const point of sampleLine(line,context,paperMm(d.styleSpacingMm,context.scale))){if(d.style==='dots')elements.push({layer:layer(d.colour),markup:`<circle data-decoration-symbol="${symbol}" cx="${point.x}" cy="${point.y}" r="${lineWidth(d.styleDiameterMm)/2}" fill="${stroke(d.colour)}"/>`});else if(feature.properties?.tagSide){const side=feature.properties.tagSide==='right'?1:-1,angle=point.angle+side*Number(d.tagAngleDeg||60)*Math.PI/180,len=lineWidth(d.tagLengthMm),dx=Math.cos(angle)*len,dy=Math.sin(angle)*len;elements.push({layer:layer(d.colour),markup:`<path data-decoration-symbol="${symbol}" d="M${point.x},${point.y}L${point.x+dx},${point.y+dy}" stroke="${stroke(d.colour)}" stroke-width="${lineWidth(d.tagWidthMm)}" stroke-linecap="butt"/>`})}}
    return elements;
  }
