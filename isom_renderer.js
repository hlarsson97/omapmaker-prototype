(function(root){
  ...
  function lineStyles(symbol,properties={},context){
    ...
    if(d.kind==='configurable-boundary'&&properties.variant==='black-dots')return{outer:{color:colour(d.dotColour),weight:px(d.dotDiameterMm,context),opacity:1,dashArray:`0 ${px(d.dotRepeatMm-d.dotDiameterMm,context)}`,lineCap:'round',lineJoin:'round'}};
    ...
    if(d.kind==='double-line-with-supports'||d.kind==='double-line'){
    ...
    if(d.kind==='opening-line'&&properties.runnability&&properties.runnability!=='surrounding'){const backgrounds={better:'yellow',normal:'white',slow:'green30',walk:'green60'};return{background:{...base,color:colour(backgrounds[properties.runnability]||'white'),weight:px(d.backgroundWidthMm,context),dashArray:null},outer:base}}
    return{outer:base};
  }
  function areaStyle(symbol,properties={},context){
    ...
    const fill=large&&d.largeFill?d.largeFill:d.screenPreview||properties.background||d.fill,hasFill=Boolean(fill)&&fill!=='none';
  ...
  function pointMarkup(symbol,context,properties={}){
    ...
    else if(d?.kind==='point-star'){const arms=[];for(let i=0;i<3;i++){const a=i*Math.PI/3,dx=Math.cos(a)*w/2,dy=Math.sin(a)*h/2;arms.push(`M${-dx},${-dy}L${dx},${dy}`)}body=`<path d="${arms.join(' ')}" fill="none" stroke="${stroke}" stroke-width="${sw}" stroke-linecap="round"/>`}
    ...
    else if(d?.kind==='registration-mark')body=`<path d="M${-w/2},0H${w/2} M0,${-h/2}V${h/2}" fill="none" stroke="${stroke}" stroke-width="${sw}"/>`;
    else if(d?.kind==='elevation-text'){const label=svgEscape(properties.elevation??properties.heightMetres??properties.mapText??'0'),dot=properties.waterSurface?'':`<circle cx="${-w*.32}" cy="0" r="${paperMm(d.dotDiameterMm,context.scale)/2}" fill="${stroke}"/>`;body=`${dot}<text x="${w*.05}" y="${paperMm(d.textHeightMm,context.scale)*.35}" font-family="${svgEscape(registry.textRules.fontFamily)}" font-size="${paperMm(d.textHeightMm,context.scale)}" text-anchor="middle" fill="${stroke}">${label}</text>`}
    else if(d?.kind==='course-start'||d?.kind==='map-exchange-start'){const rotation=number(properties.orientationDegrees)-number(context.declination),radius=(d.kind==='course-start'?Math.min(w,h*1.155):w)/2,points=[-Math.PI/2,-Math.PI/2+2*Math.PI/3,-Math.PI/2+4*Math.PI/3].map(a=>`${Math.cos(a)*radius},${Math.sin(a)*radius}`).join(' '),triangle=`<polygon points="${points}" fill="none" stroke="${stroke}" stroke-width="${sw}"/>`;body=d.kind==='map-exchange-start'?`<g transform="rotate(${rotation})"><circle cx="0" cy="0" r="${w/2-sw/2}" fill="none" stroke="${stroke}" stroke-width="${sw}"/>${triangle}</g>`:`<g transform="rotate(${rotation})">${triangle}</g>`}
    else if(d?.kind==='map-issue-point')body=`<path d="M0,${-h/2}V${h/2}" stroke="${stroke}" stroke-width="${sw}"/>`;
    else if(d?.kind==='control-circle')body=`<circle cx="0" cy="0" r="${w/2-sw/2}" fill="none" stroke="${stroke}" stroke-width="${sw}"/>`;
    else if(d?.kind==='course-number'){const label=svgEscape(properties.controlNumber??properties.mapText??'1');body=`<text x="0" y="${paperMm(d.textHeightMm,context.scale)*.35}" font-family="Arial, sans-serif" font-size="${paperMm(d.textHeightMm,context.scale)}" text-anchor="middle" fill="${stroke}">${label}</text>`}
    else if(d?.kind==='course-finish'){const inner=paperMm(d.innerDiameterMm,context.scale);body=`<circle cx="0" cy="0" r="${w/2-sw/2}" fill="none" stroke="${stroke}" stroke-width="${sw}"/><circle cx="0" cy="0" r="${inner/2-sw/2}" fill="none" stroke="${stroke}" stroke-width="${sw}"/>`}
    else if(d?.kind==='crossing-gate'){const rotation=number(properties.orientationDegrees)-number(context.declination),curve=paperMm(.45,context.scale);body=`<g transform="rotate(${rotation})"><path d="M${-w/2},${-h/2} C${-w/2-curve},${-h*.2} ${-w/2-curve},${h*.2} ${-w/2},${h/2} M${w/2},${-h/2} C${w/2+curve},${-h*.2} ${w/2+curve},${h*.2} ${w/2},${h/2}" fill="none" stroke="${stroke}" stroke-width="${sw}"/></g>`}
    else if(d?.kind==='first-aid'){const bw=paperMm(d.barWidthMm,context.scale);body=`<path d="M${-bw/2},${-h/2}H${bw/2}V${-bw/2}H${w/2}V${bw/2}H${bw/2}V${h/2}H${-bw/2}V${bw/2}H${-w/2}V${-bw/2}H${-bw/2}Z" fill="${stroke}"/>`}
    else if(d?.kind==='refreshment'){const base=paperMm(d.baseWidthMm,context.scale);body=`<path d="M${-w/2},${-h/2} H${w/2} L${base/2},${h*.35} H${-base/2} Z M${-base/2},${h/2}H${base/2}" fill="none" stroke="${stroke}" stroke-width="${sw}" stroke-linejoin="round"/>`}
    ...
    else if(d?.kind==='double-line-with-supports')body=largeSupport?`<path d="M${-w/2},0H${w/2}" stroke="${stroke}" stroke-width="${sw}"/><rect x="${-largeSize/2}" y="${-largeSize/2}" width="${largeSize}" height="${largeSize}" fill="none" stroke="${stroke}" stroke-width="${sw}"/>`:`<path d="M${-w/2},0H${w/2}" stroke="${stroke}" stroke-width="${sw}"/>`;
    else body=`<circle cx="0" cy="0" r="${Math.max(.1,w/3)}" fill="${stroke}"/>`;
    if((d?.kind==='line-with-supports'||d?.kind==='double-line-with-supports')&&Number.isFinite(Number(properties.angleDegrees)))body=`<g transform="rotate(${90-number(properties.angleDegrees)-number(context.declination)})">${body}</g>`;
  ...
  function layoutPointSymbols(features, options={}) {
    ...
    const cellSize=32, cells=new Map(), broad=[], solids=[];
    function inRing(point,ring){let inside=false;for(let i=0,j=ring.length-1;i<ring.length;j=i++){const a=ring[i],b=ring[j];if((a.y>point.y)!==(b.y>point.y)&&point.x<(b.x-a.x)*(point.y-a.y)/(b.y-a.y)+a.x)inside=!inside;}return inside;}
    function insert(obstacle) {
      const {left,right,bottom,top}=obstacle;
      const x0=Math.floor(left/cellSize),x1=Math.floor(right/cellSize),y0=Math.floor(bottom/cellSize),y1=Math.floor(top/cellSize);
      if((x1-x0+1)*(y1-y0+1)>256){broad.push(obstacle);return;}
      for(let x=x0;x<=x1;x++)for(let y=y0;y<=y1;y++){const key=`${x}:${y}`;if(!cells.has(key))cells.set(key,[]);cells.get(key).push(obstacle);}
    }
    ...
    for(const feature of active){
      ...
      if(polygon&&['206','521'].includes(symbol))for(const polygonCoordinates of geometry.type==='Polygon'?[geometry.coordinates]:geometry.coordinates){const rings=polygonCoordinates.map(ring=>ring.map(project)),bounds={rings,left:Infinity,right:-Infinity,bottom:Infinity,top:-Infinity};for(const ring of rings)for(const p of ring){bounds.left=Math.min(bounds.left,p.x);bounds.right=Math.max(bounds.right,p.x);bounds.bottom=Math.min(bounds.bottom,p.y);bounds.top=Math.max(bounds.top,p.y);}solids.push(bounds);}
      let width;
      if(polygon)width=outlined?number(d.outlineWidthMm)*unit:0;
      else if(options.map&&options.mode&&options.mode!=='print'){const context={map:options.map,scale:options.scale||15000,mode:options.mode};width=lineStyles(symbol,feature.properties,context).outer.weight*metresPerPixel(options.map);}
      else if(d.kind==='cased-line')width=Math.max(number(d.minimumInnerWidthMm)*unit,number(feature.properties?.widthMetres||feature.properties?.renderWidthMetres||feature.properties?.estimatedWidthMetres))+2*number(d.outlineWidthMm)*unit;
      else width=number(d.widthMm||d.lineWidthMm||d.dotDiameterMm||.14)*unit;
      // Conservative envelope for decorations, avoiding a move onto fence tags etc.
    ...
    for(const point of ordered){
      ...
      function valid(candidate){
        if(nearSolids.some(({rings})=>inRing(candidate,rings[0])&&!rings.slice(1).some(ring=>inRing(candidate,ring))))return false;
        for(const {o,sign} of sides.values()){const cross=(o.b.x-o.a.x)*(candidate.y-o.a.y)-(o.b.y-o.a.y)*(candidate.x-o.a.x);if(Math.abs(sign)>1e-5&&sign*cross<0)return false;}
        return obstacles.every(o=>{const n=nearest(candidate,o);return Math.hypot(candidate.x-n.x,candidate.y-n.y)+1e-7>=radius+gap+o.radius;});
      }
      let best=valid(p)?p:null,bestDistance=best?0:Infinity;
      function consider(candidate){const distance=Math.hypot(candidate.x-p.x,candidate.y-p.y);if(distance<=maxShift&&distance<bestDistance&&valid(candidate)){best=candidate;bestDistance=distance;}}
      if(!best){
        for(const o of obstacles){const n=nearest(p,o),dx=p.x-n.x,dy=p.y-n.y,length=Math.hypot(dx,dy),required=radius+gap+o.radius+1e-5;
          if(length>1e-7)consider({x:n.x+dx/length*required,y:n.y+dy/length*required});
          else if(o.a){const vx=o.b.x-o.a.x,vy=o.b.y-o.a.y,len=Math.hypot(vx,vy)||1;for(const side of [1,-1])consider({x:n.x-vy/len*required*side,y:n.y+vx/len*required*side});}
        }
        // Intersections and clusters: deterministic bounded search, starting nearest.
  ...
  function preflight(features,options){
    ...
    (features||[]).forEach((feature,index)=>{
      ...
      if(d.minimumSymbols&&length!=null){const repeat=paperMm(d.styleSpacingMm||d.patternSpacingMm||1,scale),count=repeat?Math.floor(length/repeat):0;if(count<d.minimumSymbols)issues.push({severity:'warning',code:'minimum-symbols',symbol,featureId:feature.id||null,message:`${label}: för få grafiska tecken för att symbolen ska kännas igen.`})}
      ...
      if(d.requiresOrientation&&!Number.isFinite(Number(feature.properties?.orientationDegrees)))issues.push({severity:'warning',code:'orientation-missing',symbol,featureId:feature.id||null,message:`${label}: riktning saknas, så symbolen kan inte orienteras normriktigt.`});
      if(d.requiresDefinition&&!String(feature.properties?.legendDefinition||'').trim())issues.push({severity:'error',code:'definition-missing',symbol,featureId:feature.id||null,message:`${label}: symbolens betydelse saknas i teckenförklaringen.`});
  ...
  function patternDefs(scale){
    const f=factor(scale),c=colour;return[
      ...
      ...[['random-dots',.55],['random-dots-dense',.36],['random-dots-very-dense',.285]].flatMap(([name,spacing])=>['brown','black'].map(ink=>`<pattern id="p-${name}-${ink}" patternUnits="userSpaceOnUse" width="${spacing*2*f}" height="${spacing*2*f}"><circle cx="${spacing*.35*f}" cy="${spacing*.42*f}" r="${.1*f}" fill="${c(ink)}"/><circle cx="${spacing*1.45*f}" cy="${spacing*.9*f}" r="${.1*f}" fill="${c(ink)}"/><circle cx="${spacing*.8*f}" cy="${spacing*1.55*f}" r="${.1*f}" fill="${c(ink)}"/></pattern>`)),
      `<pattern id="p-boulders" patternUnits="userSpaceOnUse" width="${2*f}" height="${1.2*f}"><path d="M${.1*f},${.1*f} L${.9*f},${.25*f} L${.3*f},${.7*f}Z" fill="${c('black')}"/><path d="M${1.1*f},${.65*f} L${1.85*f},${.5*f} L${1.55*f},${1.15*f}Z" fill="${c('black')}"/></pattern>`,
      `<pattern id="p-boulders-dense" patternUnits="userSpaceOnUse" width="${1.1*f}" height="${.8*f}"><path d="M${.05*f},${.1*f} L${.85*f},${.18*f} L${.3*f},${.7*f}Z" fill="${c('black')}"/><path d="M${.65*f},${.65*f} L${1.05*f},${.3*f} L${.95*f},${.78*f}Z" fill="${c('black')}"/></pattern>`,
      `<pattern id="p-sand" patternUnits="userSpaceOnUse" width="${.45*f}" height="${.45*f}" patternTransform="rotate(45)"><rect width="100%" height="100%" fill="${c('yellow50')}"/><circle cx="${.225*f}" cy="${.225*f}" r="${.08*f}" fill="${c('black')}"/></pattern>`,
      ...['yellow','yellow50'].flatMap(background=>[
        `<pattern id="p-orchard-${background}" patternUnits="userSpaceOnUse" width="${.8*f}" height="${.8*f}"><rect width="100%" height="100%" fill="${c(background)}"/><circle cx="${.4*f}" cy="${.4*f}" r="${.225*f}" fill="${c('green')}"/></pattern>`,
        `<pattern id="p-vineyard-${background}" patternUnits="userSpaceOnUse" width="${.85*f}" height="${1.9*f}"><rect width="100%" height="100%" fill="${c(background)}"/><path d="M${.1*f},0V${1.3*f}" stroke="${c('green')}" stroke-width="${.2*f}"/></pattern>`]),
      `<pattern id="p-forbidden-crosshatch" patternUnits="userSpaceOnUse" width="${1.2*f}" height="${1.2*f}" patternTransform="rotate(45)"><path d="M0 0V${1.2*f} M${.6*f} 0V${1.2*f}" stroke="${c('purpleUpper')}" stroke-width="${.2*f}"/></pattern>`
    ].join('');
  }
  function areaFill(symbol,d,properties={}){if(symbol==='307'||symbol==='308')return'url(#p-marsh-solid)';if(symbol==='310')return'url(#p-marsh-dashed)';if(symbol==='402')return'url(#p-scattered-402)';if(symbol==='404')return'url(#p-scattered-404)';if(symbol==='407')return'url(#p-vertical-green-407)';if(symbol==='409')return'url(#p-vertical-green-409)';if(symbol==='412')return'url(#p-cultivated)';if(['113','114','210','211','212'].includes(symbol))return`url(#p-${d.pattern}-${d.patternColour})`;if(symbol==='208')return'url(#p-boulders)';if(symbol==='209')return'url(#p-boulders-dense)';if(symbol==='213')return'url(#p-sand)';if(symbol==='413')return`url(#p-orchard-${properties.background==='yellow50'?'yellow50':'yellow'})`;if(symbol==='414')return`url(#p-vineyard-${properties.background==='yellow50'?'yellow50':'yellow'})`;if(symbol==='709')return'url(#p-forbidden-crosshatch)';return colour(properties.background||d.fill||'white')}
  ...
  function vectorElements(feature,context){
    ...
    if(feature.geometry.type==='Point'){
      ...
      if(d.allPrintingColours){for(const ink of registry.colourOrder.filter(name=>!['white','black20','black35','black50','yellow75','yellow50','green30','green60','blue50','blue70','brown50','purpleLower','purpleUpper'].includes(name)))elements.push({layer:layer(ink),markup:`<svg x="${p.x-w/2-pad}" y="${p.y-h/2-pad}" width="${w+2*pad}" height="${h+2*pad}" viewBox="${-w/2-pad} ${-h/2-pad} ${w+2*pad} ${h+2*pad}" overflow="visible">${markup.replaceAll(colour(d.colour||'black'),colour(ink))}</svg>`});return elements}
    ...
    if(d.kind==='area'||d.kind==='pattern-area'||d.kind==='outline-area'){
      const showOutline=d.outline&&(!d.outlineConditional||feature.properties?.boundary==='clear'),large=d.largeThresholdMetres&&number(feature.properties?.maximumDimensionMetres)>=d.largeThresholdMetres,fillLayer=layer(large&&d.largeFill?d.largeFill:d.fill||d.patternColour||'white');let fill=large&&d.largeFill?colour(d.largeFill):areaFill(symbol,d,feature.properties||{});
      if(['413','414'].includes(symbol)&&Number.isFinite(Number(feature.properties?.orientationDegrees))){const angle=number(feature.properties.orientationDegrees),safe=String(feature.id||`${symbol}-${path}`).replace(/[^a-zA-Z0-9_-]/g,'').slice(-40),id=`p-${symbol}-${safe}`,spacing=paperMm(d.patternSpacingMm,context.scale),background=colour(feature.properties?.background==='yellow50'?'yellow50':'yellow'),ink=colour(d.patternColour);let patternMarkup;if(symbol==='413'){const dot=paperMm(d.dotDiameterMm,context.scale);patternMarkup=`<pattern id="${id}" patternUnits="userSpaceOnUse" width="${spacing}" height="${spacing}" patternTransform="rotate(${angle})"><rect width="100%" height="100%" fill="${background}"/><circle cx="${spacing/2}" cy="${spacing/2}" r="${dot/2}" fill="${ink}"/></pattern>`}else{const height=paperMm(1.9,context.scale),width=paperMm(d.patternLineWidthMm,context.scale),dash=(d.rowDashMm||[]).map(value=>paperMm(value,context.scale)).join(' ');patternMarkup=`<pattern id="${id}" patternUnits="userSpaceOnUse" width="${spacing}" height="${height}" patternTransform="rotate(${angle})"><rect width="100%" height="100%" fill="${background}"/><path d="M${width/2},0V${paperMm(1.3,context.scale)}" stroke="${ink}" stroke-width="${width}"${dash?` stroke-dasharray="${dash}"`:''}/></pattern>`}elements.push({layer:fillLayer,markup:`<defs>${patternMarkup}</defs>`});fill=`url(#${id})`}
      elements.push({layer:fillLayer,markup:`<path d="${path}" fill="${fill}" fill-rule="evenodd"/>`});
    ...
    if(d.kind==='opening-line'&&feature.properties?.runnability&&feature.properties.runnability!=='surrounding'){const backgrounds={better:'yellow',normal:'white',slow:'green30',walk:'green60'},background=backgrounds[feature.properties.runnability]||'white';elements.push({layer:layer(background),markup:`<path data-composite-symbol="${symbol}" d="${path}" fill="none" stroke="${stroke(background)}" stroke-width="${lineWidth(d.backgroundWidthMm)}" stroke-linejoin="round"/>`})}
    if(d.kind==='configurable-boundary'&&feature.properties?.variant==='black-dots'){elements.push({layer:layer(d.dotColour),markup:`<path d="${path}" fill="none" stroke="${stroke(d.dotColour)}" stroke-width="${lineWidth(d.dotDiameterMm)}" stroke-dasharray="0 ${lineWidth(d.dotRepeatMm-d.dotDiameterMm)}" stroke-linecap="round"/>`});return elements}
    ...
    if(d.kind==='bridge-tunnel')for(const line of coordinatesForGeometry(feature.geometry)){const projected=line.map(p=>paperProject(p,context));if(projected.length<2)continue;const tag=lineWidth(d.tagLengthMm),angle=number(d.tagAngleDeg||60)*Math.PI/180,along=Math.cos(angle)*tag,across=Math.sin(angle)*tag,outer=(point,inside,isStart)=>{const dx=isStart?inside.x-point.x:point.x-inside.x,dy=isStart?inside.y-point.y:point.y-inside.y,length=Math.hypot(dx,dy)||1,axis={x:dx/length,y:dy/length},normal={x:axis.y,y:-axis.x},outward=isStart?-1:1;return{x:point.x+axis.x*outward*along+normal.x*across,y:point.y+axis.y*outward*along+normal.y*across}},points=[outer(projected[0],projected[1],true),...projected,outer(projected.at(-1),projected.at(-2),false)],path=points.map((point,index)=>`${index?'L':'M'}${point.x},${point.y}`).join(' ');elements.push({layer:layer(d.colour),markup:`<path data-decoration-symbol="${symbol}" d="${path}" fill="none" stroke="${stroke(d.colour)}" stroke-width="${lineWidth(d.tagWidthMm||d.widthMm)}" stroke-linecap="butt" stroke-linejoin="miter"/>`})}
    if(d.kind==='styled-line')for(const line of coordinatesForGeometry(feature.geometry)){
      ...
      for(const point of points){
        ...
        else if(d.style==='course-crosses'){const halfW=lineWidth(d.styleWidthMm)/2,halfH=lineWidth(d.styleHeightMm)/2,cos=Math.cos(point.angle),sin=Math.sin(point.angle),rotate=(x,y)=>({x:point.x+x*cos-y*sin,y:point.y+x*sin+y*cos}),a=rotate(-halfW,-halfH),b=rotate(halfW,halfH),c=rotate(-halfW,halfH),e=rotate(halfW,-halfH);elements.push({layer:layer(d.colour),markup:`<path data-decoration-symbol="${symbol}" d="M${a.x},${a.y}L${b.x},${b.y} M${c.x},${c.y}L${e.x},${e.y}" stroke="${stroke(d.colour)}" stroke-width="${lineWidth(d.widthMm)}"/>`})}
      }
    }
    return elements;
  }
